...
public class JobIntentServiceSaveTicket extends JobIntentService {
    ...
    protected synchronized void onHandleWork(@NonNull Intent intent) {
        ...
        try {
            ...
            // Sync the devices if the list is not empty
    ...
    private static void updateTicketStatus(TicketSuccess response, String status) {
        try {
            if (response != null && response.getResult()) {
                ...
                log.info("Ticket Save on server3=====>" + response.getCitationNumber());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static void __updateTicketPictureImageStatus(int sNo, long citationNumber, boolean uploadFlag) {
        ...
        System.out.println("imageStatus2>>>>>>>>>>"+imageStatus);
