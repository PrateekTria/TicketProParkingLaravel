...
public abstract class ParkingDatabase extends RoomDatabase {
    ...
    public static final Migration MIGRATION_23_24 = new Migration(23, 24) {
        ...
        public void migrate(@NonNull SupportSQLiteDatabase database) {
            // Execute the SQL query to alter the table and make the primary key auto-generate
            try {
                database.execSQL(
                        "CREATE TABLE IF NOT EXISTS chalk_comments_new (" +
                                "chalk_comment_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL," +
                                "chalk_id INTEGER," +
                                "comment_id INTEGER," +
                                "comment TEXT," +
                                "is_private TEXT DEFAULT 'N'," +
                                "custid INTEGER" +
                                ")"
                );
                ...
                database.execSQL(
                        "INSERT INTO chalk_comments_new (chalk_id, comment_id, comment, is_private, custid)" +
                                "SELECT chalk_id, comment_id, comment, is_private, custid" +
                                "FROM chalk_comments"
                );

                // Drop the old table
                database.execSQL("DROP TABLE chalk_comments");

                // Rename the new table to the original table name
                database.execSQL("ALTER TABLE chalk_comments_new RENAME TO chalk_comments");
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
