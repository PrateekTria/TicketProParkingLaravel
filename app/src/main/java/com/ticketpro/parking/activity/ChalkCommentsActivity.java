...
public class ChalkCommentsActivity extends BaseActivityImpl{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			if(data.hasExtra("Comment")){
				...
				boolean duplicateComment = data.getBooleanExtra("duplicateComment",false);
				...
				System.out.println("commentList1>>>>>>>>>>>"+ Arrays.toString(activeChalk.getComments().toArray()));


				if(isDuplicateComment(activeChalk.getComments(),comment)){
		...
		if(activeChalk!=null && activeChalk.getComments().isEmpty()){
			setResult(RESULT_OK);
			finish();
		}
	}
	
	private boolean isDuplicateComment(List<ChalkComment> commentList, String comment){
		boolean result=false;
		System.out.println("commentList2>>>>>>>>>>>"+ Arrays.toString(commentList.toArray()));
		for(ChalkComment chalkComment:commentList){
			System.out.println(chalkComment.getCommentId());
			// This has been Modified by Prateek 1/may/2024
			if(chalkComment.getComment().equals(comment)){
				return true;
			}
		}
	...
	public class ListAdapter extends ArrayAdapter<ChalkComment>{
	    ...
	    public View getView(final int position, View convertView, ViewGroup parent){
			...
			if(items.get(position).isPrivateComment()){
				itemView.setBackgroundResource(R.drawable.color_red);
				deleteBtn.setColorFilter(getResources().getColor(R.color.white));
