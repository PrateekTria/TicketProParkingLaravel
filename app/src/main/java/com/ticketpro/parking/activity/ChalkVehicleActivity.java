...
public class ChalkVehicleActivity extends BaseActivityImpl implements MyTracker.ADLocationListener, MeterHandler {
    ...
    protected void onResume() {
       ...
       // __startChalkWork();
        ...
        System.out.println("Comments>>>>>>>>>>>>>>>>>>>>"+activeChalk.getComments());
        activeChalk.setChalkPictures(TPApp.getLastChalkPhotos());
    ...
    public void addChalkAction(View view) {
        try {
            ...
            activeChalk.setUserId(TPApp.getCurrentUserId());
            //This code has been added by Prateek on 14/5/2024
            activeChalk.setIsExpired("N");
            ...
            List<ChalkComment> comments = activeChalk.getComments();
            System.out.println("activeChalkcomment>>>>"+comments);

            ChalkComment cc = new ChalkComment();
            int chalkCommentId = 0;
            ArrayList<ChalkComment> chalkComment = activeChalk.getComments();

            // Insert Chalk Comments
            for (ChalkComment comment : chalkComment) {
                ChalkComment.insertChalkComment(comment).subscribe();

            //     cc.setChalkCommentId(activeTicket.getTicketId());
                cc.setCommentId(comment.getCommentId());
                cc.setCustId(TPApp.custId);
                cc.setChalkId(comment.getChalkId());
                cc.setVoiceComment(false);
                cc.setIsPrivate(comment.getIsPrivate());
                cc.setComment(comment.getComment());
                ChalkComment.insertChalkComment(cc);
            ...
            activeChalk.getChalkPictures().clear();
          //  TPApp.setActiveChalk(activeChalk);
//            if (TPApp.stickyChalkPhoto) {
//                Iterator<ChalkPicture> iterator = TPApp.getLastChalkPhotos().iterator();
//                while (iterator.hasNext()) {
//                    ChalkPicture next = iterator.next();
//                    next.setChalkId(activeChalk.getChalkId());
//                    if (!next.isPhotoSp()) {
//                        iterator.remove();
//                    }
//                }
//                activeChalk.setChalkPictures(TPApp.getLastChalkPhotos());
//            }
//            if (!activeChalk.getChalkPictures().isEmpty()) {
//
//                pictureButton.setText("Pictures(" + activeChalk.getChalkPictures().size() + ")");
//            } else {
